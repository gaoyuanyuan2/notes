...
索引尽量用自增字段，索引树效率高，但是会暴露业务情况，可以用雪花算法解决此问题
...
4.MVCC排他锁(写) 行级锁  多个版本的数据  MVCC主要适用于Mysql的RC,RR隔离级别 (无非就是乐观锁的一种实现方式)

不再对查询加锁了提供了查询并发能力

三个隐藏字段:

DB_ROW_ID, 如果表中没有显示定义主键或者没有唯一素引则MySQI会自动创建.一个6字节的row id存在记录中

DB_TRX_ID,事务ID

DB_ROLL_PTR，回滚段指针

隔离是通过并发控制协议实现的。最简单的方法是让所有读者等到编写器完成，这称为读写锁。众所周知，锁会在长读事务和更新事务之间产生争用。MVCC旨在通过保留每个数据项的多个副本来解决问题。
以这种方式，连接到数据库的每个用户在特定时刻看到数据库的快照。在更改完成之前（或者，在数据库术语中：直到事务已提交之前），数据库的其他用户将看不到编写者所做的任何更改。

当MVCC数据库需要更新一段数据时，它不会用新数据覆盖原始数据项，而是创建数据项的较新版本。

因此存储了多个版本。每个事务看到的版本取决于实现的隔离级别。

使用MVCC实现的最常见隔离级别是快照隔离。通过快照隔离，事务会在事务启动时观察数据状态。MVCC引入了如何删除过时且永远不会被读取的版本的挑战。

在某些情况下，实现了定期扫描和删除过时版本的过程。这通常是一个遍历全局的进程，遍历整个表并使用每个数据项的最后一个版本重写它。

PostgreSQL采用这种方法及其VACUUM流程。其他数据库将存储块拆分为两部分：数据部分和撤消日志。数据部分始终保留最后提交的版本。撤消日志可以重新创建旧版本的数据。后一种方法的主要固有限制是，当存在更新密集型工作负载时，撤消日志部分用尽空间，然后由于无法为其提供快照而中止事务。

对于面向文档的数据库，它还允许系统通过将整个文档写入磁盘的连续部分来优化文档 - 更新后，整个文档可以重新编写，而不是在链接的非切割或维护连续的数据库结构。


MVCC提供时间点一致的视图。MVCC下的读取事务通常使用时间戳或事务ID来确定要读取的DB的状态，并读取这些版本的数据。因此，读取和写入事务彼此隔离，而无需锁定。但是，尽管不需要锁定，但某些MVCC数据库（如Oracle）会使用它们。写入创建较新版本，而并发读取访问旧版本。
